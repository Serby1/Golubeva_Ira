...
namespace _22._2
{
    class Graph
    {
        ...
        public void SearchG() //во внешнем классе
        {
            ...
            int[] mas = new int[c.Count];
            int k = 0;
            while (c.Count != 0)
            {
                mas[k] = (int)c.Pop();
                Console.Write("{0} ", mas[k]);
                ++k;
            }
            Console.WriteLine();
            int max = 0;
            int nmax = 0;
            int m = 0;
            // перебираю все номера вершин и смотрю, сколько раз они встречаются
            Console.WriteLine("Количество  проходов по вершинам соответсвенно:");
            for (int i = 0; i < graph.Size; ++i)
            {
                for (int j = 0; j < k; ++j)
                {
            ...
            Console.WriteLine();
            Console.Write("Самая часто встречаемая вершина в пути: {0} ", nmax);
        }
